...
# "%" must be escaped as "\%" under real LaTeX (it's a comment character
# there) but must NOT be escaped under matplotlib's built-in mathtext
# fallback, where a literal backslash would just print. Pick the right
# symbol once, based on which renderer is actually active.
...
def figure_scaling(records, out_dir):
    ...
    # All panels share the same set of noise strengths, so a single legend
    # (built from the first panel's handles) covers every panel -- no need
    # to repeat the same p=0.02...0.08 key four times.
    ...
    # Redraw the shared legend's handles in neutral gray/black so it reads
    # as "these apply to every panel," not "these belong to panel 1."
    import matplotlib.lines as mlines
    neutral_handles = []
    for h, lab in zip(shared_handles, shared_labels):
        if lab == r"$R_{\rm exact}$":
            neutral_handles.append(mlines.Line2D([], [], color=EXACT_COLOR,
                                                    linewidth=0.85, linestyle=(0, (3, 2))))
        else:
            neutral_handles.append(mlines.Line2D([], [], color="0.35", marker="o",
                                                    markersize=5, linewidth=0))

    fig.legend(neutral_handles, shared_labels, loc="lower center",
                ncol=len(shared_labels), frameon=False, fontsize=8.6,
...
def figure_error_and_r2(records, out_dir):
    ...
    ax1.set_ylabel(rf"Relative error ({PCT})")
    _style_axes(ax1)
...
# ---------------------------------------------------------------------------
# Figure 5: ancilla-local vs system-noise head-to-head (same physical noise
# type, two locations) -- the single clearest statement of the paper's
# central claim, meant to work almost on its own as an abstract-level figure.
# ---------------------------------------------------------------------------

# Map each "ancilla" channel to the "system" channel it should be compared
# against (same underlying physical noise type, applied in the two
# different locations). Edit this if your channel naming differs.
HEAD_TO_HEAD_PAIRS = [("phase_flip", "dephasing")]


def figure_head_to_head(records, out_dir):
    pairs = [
        (a, s) for a, s in HEAD_TO_HEAD_PAIRS
        if any(r["channel"] == a and r["scope"] == "ancilla" for r in records)
        and any(r["channel"] == s and r["scope"] == "system" for r in records)
    ]
    if not pairs:
        print("figure_head_to_head: no matching ancilla/system pair found, skipping.")
        return

    fig, ax = plt.subplots(figsize=(6.0, 4.2))
    fig.subplots_adjust(left=0.13, right=0.97, bottom=0.14, top=0.90)

    for anc_channel, sys_channel in pairs:
        rs_anc = sorted([r for r in records if r["channel"] == anc_channel
                          and r["scope"] == "ancilla"], key=lambda r: r["strength"])
        rs_sys = sorted([r for r in records if r["channel"] == sys_channel
                          and r["scope"] == "system"], key=lambda r: r["strength"])

        p_anc = np.array([r["strength"] for r in rs_anc])
        exact_anc = np.array([r["exact"] for r in rs_anc])
        err_anc = np.clip(100 * np.abs(np.array([r["RZNE"] for r in rs_anc]) - exact_anc)
                           / np.abs(exact_anc), ERROR_FLOOR, None)

        p_sys = np.array([r["strength"] for r in rs_sys])
        exact_sys = np.array([r["exact"] for r in rs_sys])
        err_sys = np.clip(100 * np.abs(np.array([r["RZNE"] for r in rs_sys]) - exact_sys)
                           / np.abs(exact_sys), ERROR_FLOOR, None)

        ax.plot(p_anc, err_anc, marker="D", markersize=5.5, linewidth=1.5,
                 color=COLORS.get(anc_channel, "#CC79A7"),
                 label="Ancilla-local (eigenoperator)")
        ax.plot(p_sys, err_sys, marker="s", markersize=5.5, linewidth=1.5,
                 color=COLORS.get(sys_channel, "#009E73"),
                 label="System, under folding (operator-mixing)")

    ax.set_yscale("log")
    ax.set_xlabel(r"Physical noise strength $p$")
    ax.set_ylabel(rf"R-ZNE relative error ({PCT})")
    _style_axes(ax)
    ax.legend(**legend_kwargs(loc="center left"))

    save_figure(fig, out_dir, "fig5_ancilla_vs_system")


# ---------------------------------------------------------------------------
# Figure 6: compact all-channel summary (single panel) + improvement factor
# ---------------------------------------------------------------------------

def figure_summary(records, out_dir):
    channels = ordered_channels(records)

    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(8.6, 3.6))
    fig.subplots_adjust(left=0.09, right=0.98, bottom=0.16, top=0.90, wspace=0.28)
    ...
    for channel in channels:
        ...
        raw_err = np.clip(100 * np.abs(raw - exact) / np.abs(exact), ERROR_FLOOR, None)
        mit_err = np.clip(100 * np.abs(mit - exact) / np.abs(exact), ERROR_FLOOR, None)

        color = COLORS[channel]
        marker = MARKERS[channel]

        # Left: all four channels' R-ZNE error on one shared axis.
        ax1.plot(p, mit_err, marker=marker, markersize=4.6, linewidth=1.4,
                  color=color, label=LABELS[channel])

        # Right: improvement factor = raw error / R-ZNE error.
        improvement = raw_err / mit_err
        ax2.plot(p, improvement, marker=marker, markersize=4.6, linewidth=1.4,
                  color=color, label=LABELS[channel])

    ax1.set_yscale("log")
    ax1.set_xlabel(r"Physical noise strength $p$")
    ax1.set_ylabel(rf"R-ZNE relative error ({PCT})")
    _style_axes(ax1)
    ax1.legend(**legend_kwargs(loc="lower right"))

    ax2.set_yscale("log")
    ax2.set_xlabel(r"Physical noise strength $p$")
    ax2.set_ylabel(r"Improvement factor, raw / R-ZNE")
    _style_axes(ax2)
    ax2.legend(**legend_kwargs(loc="best"))

    save_figure(fig, out_dir, "fig6_summary")
...
def main():
    ...
    figure_head_to_head(records, args.out_dir)
    figure_summary(records, args.out_dir)
